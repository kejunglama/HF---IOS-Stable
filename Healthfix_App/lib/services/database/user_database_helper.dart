...
class UserDatabaseHelper {
  ...
  Future<List<String>> get addressesList async {
    String uid = AuthentificationService().currentUser.uid;
    final snapshot =
        await firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(ADDRESSES_COLLECTION_NAME).get();
  ...
  Future<Address> getAddressFromId(String id) async {
    String uid = AuthentificationService().currentUser.uid;
    final doc =
        await firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(ADDRESSES_COLLECTION_NAME).doc(id).get();
  ...
  Future<bool> addGymSubscriptionForCurrentUser(GymSubscription gymSubscription) async {
    String uid = AuthentificationService().currentUser.uid;
    final gymSubscriptionsCollectionReference =
        firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(GYM_SUBSCRIPTIONS_COLLECTION_NAME);
  ...
  Future<GymSubscription> getGymSubscriptionFromId(String id) async {
    String uid = AuthentificationService().currentUser.uid;
    final gymSubscriptionsCollectionRef =
        firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(GYM_SUBSCRIPTIONS_COLLECTION_NAME);
  ...
  Future<bool> addProductToCart(Product product, String variationId) async {
    ...
    final cartCollectionRef = firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(CART_COLLECTION_NAME);
    final docRef = isVariationPdct ? cartCollectionRef.doc(variationId) : cartCollectionRef.doc(product.id);
    final docSnapshot = await docRef.get();
    ...
    if (alreadyPresent == false) {
      ...
      // If has Variantion
        ...
        docRef.set(CartItem(itemCount: int.parse(CartItem.ITEM_COUNT_KEY) + 1).toMap());
  ...
  Future<bool> removeProductFromCart(String cartItemID) async {
    ...
    final cartCollectionReference =
        firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(CART_COLLECTION_NAME);
  ...
  Future<List<Map>> get allCartItemsList async {
    String uid = AuthentificationService().currentUser != null ? AuthentificationService().currentUser.uid : null;
    final querySnapshot =
        await firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(CART_COLLECTION_NAME).get();
    List<Map> itemsIds = [];
  ...
  Future<List<String>> get gymSubscriptionsList async {
    String uid = AuthentificationService().currentUser.uid;
    final gymSubscriptionsSnapshot =
        await firestore.collection(USERS_COLLECTION_NAME).doc(uid).collection(GYM_SUBSCRIPTIONS_COLLECTION_NAME).get();
